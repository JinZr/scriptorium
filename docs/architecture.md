...
New review output includes a structured, model-declared scope: completion status, checked and outstanding frozen source regions or PDF pages, and limitations. Submission validates declared paths and ranges against the attempt bundle. Parsing selects the supported review model from the run's frozen schema; old external review outputs retain their original two-field contract on replay. Reports read the accepted output artifact and show declared scope apart from observed tool-return events. An unsupported frozen schema fails explicitly. Scope is not a coverage score or release-gate condition.
