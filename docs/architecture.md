...
disposable copy. Before hashing or invoking the toolchain, it rejects every
file or directory symlink that escapes that copy, including broken output
links. It removes copied recorder/PDF state and standard auxiliary files before invoking
...
- Local template resources (`.cls`, `.sty`, `.bst`, `.clo`, `.def`, `.cfg`, `.fd`,
  `.enc`, `.map`, `.bbx`, `.cbx`, `.lbx`) and fonts (`.tfm`, `.vf`, `.pfb`, `.pfa`, `.otf`, `.ttf`, `.ttc`)
...
- Fresh auxiliary outputs (`.aux`, `.toc`, `.out`, `.lof`, `.lot`, `.nav`, `.snm`,
  `.vrb`, `.bcf`, `.run.xml`) do not become editable evidence. Bibliography
  helper inputs are also collected from the freshly generated version-4
  `.fdb_latexmk` database, including local styles and bibliography databases.
  A `.bbl` is a derived output only when that database records a successful
  BibTeX/Biber rule and its output. A tracked `.bbl` used without regeneration
  remains a review input, preserving the bibliography fallback contract.
- EPS conversion PDFs are derived outputs only with a frozen EPS source, a
  newly created PDF, and matching epstopdf source/output/command log records or
  a successful latexmk EPS-to-PDF rule. The log and database are removed before
  compilation so copied helper evidence cannot authorize a derived file.
  Standard `epstopdf`/`repstopdf --outfile=... source.eps` invocations are
  supported. Unsupported or incomplete conversion evidence fails closed.
...
and classified snapshot identities, including build-only inputs. Derived outputs
include their SHA-256 digests, helper identity, and local input paths. Synthetic
...
New runs also include `navigation.json`, a deterministic source navigation aid. It lists source identities and literal section headings, labels, common reference/citation commands, captions, and graphics commands with inclusive source line ranges. Comments, inline `\verb`, and `verbatim` environments are excluded while preserving line positions. Nested braces in arguments are supported; macro expansion, conditional TeX execution, custom commands, and source-to-PDF-page mapping are not inferred. A graphics entry lists matching frozen path candidates; `target_path` is populated only for a single candidate. Ambiguous or absent candidates require direct source/page inspection. The index is not evidence authority, and reviewers still use `source-map.json` and the actual source/page contents.

Base navigation bytes and their digest are frozen in the run configuration before `create_run`, along with the role prompts and shared retrieval procedure. Preparation and recovery publish those saved bytes without invoking the currently installed indexer. Existing runs without a frozen navigation record keep their original bundles, prompts, and task identities; navigation is never backfilled. Patched verification bundles generate navigation from their own source closure at first materialization. Navigation is written and bound by the bundle manifest before the final source-map completion marker. Loading a completed bundle requires its navigation digest and source identities to match; the base bundle must additionally match its frozen navigation digest. Missing or corrupt completed navigation is an infrastructure failure, not a request to regenerate history.

Role prompts direct reviewers to inventory relevant material, read bounded ranges, follow cross-references and supplementary explanations, inspect context around searches, continue after truncated output, and seek evidence that disproves a suspected issue. Visual review requires inspecting rendered pages. The existing summary records inspected and materially unchecked scope; findings and evidence schemas, approval gates, and release decisions are unchanged. These instructions do not measure exhaustive coverage or establish an improvement in reviewer recall without comparative evaluation.
