...
Evidence has exactly two shapes. Source-line evidence requires a bare source path, inclusive line range, source digest, and verbatim quotation, and forbids `page`. Compiled-PDF evidence requires `manuscript.pdf`, a 1-based page, and a quotation, and forbids line and source-digest fields. Revision edits likewise use bare paths and may target only sources marked `text_anchorable`. The provider JSON Schema and semantic validator are both derived from the run's frozen evidence-anchor contract, so they enforce the same distinction. PDF identity is already bound by the bundle digest and manifest PDF digest and is not repeated in each evidence object.
